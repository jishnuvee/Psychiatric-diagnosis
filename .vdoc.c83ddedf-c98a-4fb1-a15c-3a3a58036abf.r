...
df$specific.disorder <- factor(df$specific.disorder,
                              levels = c("Healthy control",
                                       setdiff(unique(df$specific.disorder), "Healthy control")))
  ...
  sidebarLayout(
    sidebarPanel(
      selectInput("feature", "Choose EEG Feature:",
                 choices = names(df)[grep("AB.A", names(df))],
